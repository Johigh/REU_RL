...
#dataclass for DR parameters
@dataclass
class DRconfig:
    enabled: bool = True
    lidar_sigma_range: tuple = (0.000,0.00)
    lin_acc_sigma_range: tuple = (0.00, 0.00)
    lin_acc_clip_range: tuple = (0,0)#zeroed causes no movement - bot crashes @ 800,1600
    action_sigma_range: tuple = (0,0)#no learning at 90,180
    pos_sigma_range: tuple = (0,0)

    #Frequency maze switch
    maze_freq_sw: int = 10 #switch for every N episodes
     #load the list of mazes we want to switch through 
    maze_files: tuple = (
        r'c:\Users\Johan\OneDrive\Documents\School\FAIRIS\simulation\worlds\mazes\Experiment1\WM10.xml',
        r'c:\Users\Johan\OneDrive\Documents\School\FAIRIS\simulation\worlds\mazes\Experiment1\WM11.xml',
...
def create_env():
    return FAIRISEnv(maze_file = dr.maze_files[0], horizont=horizon, dr_config = dr) #added dr config
